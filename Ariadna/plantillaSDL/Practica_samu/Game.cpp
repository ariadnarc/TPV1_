...
Game::Game(const std::vector<TextureInfo>& textInfo)
{
	...
	ReadMap("../Practica1TPV/maps/original.txt");

	infoB = new InfoBar(arrayTexturas[SPACESHIP], this, PLAYER_LIFES);
...
void Game::Update() {
	...
	while (i < aliens.size()) {
		if (!aliens[i]->Update()) {
			score += aliens[i]->getType() == 0 ? 30 : aliens[i]->getType() == 1 ? 20 : 10;
			delete aliens[i];
...
void Game::ReadMap(const std::string mapPath) {
	...
	while (!map.eof()) {
		...
		if (map.eof()) return;//si no lo pongo la lectura se hace mal,se lee otro bunker, preguntar
